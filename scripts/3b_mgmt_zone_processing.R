...
### Convert and crop pelagic layers by species ####
...
write_hulls <- function(sp_dat, oxy_path, temp_path, sal_path){
  ...
  for(i in 1:nrow(sp_dat)) { 
    ...
    if(nrow(curr_glob) > 1){  
        ...
        mgmt_hull_folder <- list.files(paste0("data/enviro/", temp_dat$region), full.names = TRUE, pattern = temp_dat$mgmt_region)
        ...
        #min depth
        min_o2_crop <- crop(min_o2_atm, mgmt_hull, mask = TRUE)
        dir.create(paste0("data/enviro/", temp_dat$region ,"/do/atm/mgmt_hull/", curr_sp, "/min_depth"), recursive = TRUE, showWarnings = FALSE)
        writeCDF(min_o2_crop, paste0("data/enviro/", temp_dat$region ,"/do/atm/mgmt_hull/", curr_sp, "/min_depth/min_depth_o2.nc"))
      
        min_temp_crop <- crop(min_temp_rast, mgmt_hull, mask = TRUE)
        dir.create(paste0("data/enviro/", temp_dat$region ,"/temp/processed/mgmt_hull/", curr_sp, "/min_depth"), recursive = TRUE, showWarnings = FALSE)
        writeCDF(min_temp_crop, paste0("data/enviro/", temp_dat$region ,"/temp/processed/mgmt_hull/", curr_sp, "/min_depth/min_depth_temp.nc"))

        #median depth
        med_o2_crop <- crop(med_o2_atm, mgmt_hull, mask = TRUE)
        dir.create(paste0("data/enviro/", temp_dat$region ,"/do/atm/mgmt_hull/", curr_sp, "/med_depth"), recursive = TRUE, showWarnings = FALSE)
        writeCDF(med_o2_crop, paste0("data/enviro/", temp_dat$region ,"/do/atm/mgmt_hull/", curr_sp, "/med_depth/med_depth_o2.nc"))
      
        med_temp_crop <- crop(med_temp_rast, mgmt_hull, mask = TRUE)
        dir.create(paste0("data/enviro/", temp_dat$region ,"/temp/processed/mgmt_hull/", curr_sp, "/med_depth"), recursive = TRUE, showWarnings = FALSE)
        writeCDF(med_temp_crop, paste0("data/enviro/", temp_dat$region ,"/temp/processed/mgmt_hull/", curr_sp, "/med_depth/med_depth_temp.nc"))
      
        #75% quantile depth
        quant_o2_crop <- crop(quant_o2_atm, mgmt_hull, mask = TRUE)
        dir.create(paste0("data/enviro/", temp_dat$region ,"/do/atm/mgmt_hull/", curr_sp, "/quant_depth"), recursive = TRUE, showWarnings = FALSE)
        writeCDF(quant_o2_crop, paste0("data/enviro/", temp_dat$region ,"/do/atm/mgmt_hull/", curr_sp, "/quant_depth/quant_depth_o2.nc"))
      
        quant_temp_crop <- crop(quant_temp_rast, mgmt_hull, mask = TRUE)
        dir.create(paste0("data/enviro/", temp_dat$region ,"/temp/processed/mgmt_hull/", curr_sp, "/quant_depth"), recursive = TRUE, showWarnings = FALSE)
        writeCDF(quant_temp_crop, paste0("data/enviro/", temp_dat$region ,"/temp/processed/mgmt_hull/", curr_sp, "/quant_depth/quant_depth_temp.nc"))
...
sp_dat1 <- sp_dat1  %>% left_join(mgmt_zone, by = "Common.name")
...
write_hulls(sp_dat = sp_dat_nwa_p, 
            oxy_path = "data/enviro/nwa/do/processed/o2_nwa_crop_mgmt_zone.nc", 
            temp_path = "data/enviro/nwa/temp/processed/temp_nwa_crop_mgmt_zone.nc", 
            sal_path = "data/enviro/nwa/salinity/processed/sal_nwa_crop_mgmt_zone.nc")

write_hulls(sp_dat = sp_dat_nep_p, 
            oxy_path = "data/enviro/nep/do/processed/o2_nep_crop_mgmt_zone.nc",
            temp_path = "data/enviro/nep/temp/processed/temp_nep_crop_mgmt_zone.nc", 
            sal_path = "data/enviro/nep/salinity/processed/sal_nep_crop_mgmt_zone.nc")