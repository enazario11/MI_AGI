...
enviro_sim <- data.frame(lat = runif(100, min = 34, max = 42), 
                         ...
                         DO_atm = runif(100, min = 0.05, max = 0.21), #if set to 0, AGI values break
                         temp = runif(100, min = 0, max = 35))
...
#mi plot
mi_tile <- ggplot(enviro_bass, aes(x = as.factor(temp_bin), y = as.factor(DO_bin), fill = mi)) +
  geom_tile() + 
...
#agi
agi_tile <- ggplot(enviro_bass, aes(x = as.factor(temp_bin), y = as.factor(DO_bin), fill = agi)) +
  geom_tile() + 
  ylab("Dissolved oxygen (kPa)") + 
  xlab("Temperature (C)") + 
  theme_custom()
...
enviro_cod$temp_bin <- cut(enviro_cod$temp_whole, breaks = 12, labels = seq(from = 0, to = 35, by = 3))
enviro_cod$DO_bin <- cut(enviro_cod$DO_whole, breaks = 6, labels = seq(from = 5, to = 21, by = 3))


#mi plot
mi_tile <- ggplot(enviro_cod, aes(x = as.factor(temp_bin), y = as.factor(DO_bin), fill = mi)) +
  geom_tile() + 
  ylab("Dissolved oxygen (kPa)") + 
  xlab("Temperature (C)") + 
  theme_custom()
...
#agi
agi_tile <- ggplot(enviro_cod, aes(x = as.factor(temp_bin), y = as.factor(DO_bin), fill = agi)) +
  geom_tile() + 
  ylab("Dissolved oxygen (kPa)") + 
  xlab("Temperature (C)") + 
  theme_custom()
...
#bass
mi_bass_1 <- ggplot(enviro_bass, aes(x = as.factor(temp_bin), y = as.factor(DO_bin), fill = mi)) +
  geom_tile() + 
  geom_rect(data = subset(enviro_bass, mi < quantile(mi, 0.10)),
            aes(xmin = as.numeric(as.factor(temp_bin)) - 0.5, 
                xmax = as.numeric(as.factor(temp_bin)) + 0.5,
                ymin = as.numeric(as.factor(DO_bin)) - 0.5, 
                ymax = as.numeric(as.factor(DO_bin)) + 0.5), 
            fill = NA, 
            color = "#F0941F", 
            linewidth = 3,
            inherit.aes = FALSE) +
...
#cod
mi_cod_1 <- ggplot(enviro_cod, aes(x = as.factor(temp_bin), y = as.factor(DO_bin), fill = mi)) +
  geom_tile() + 
  geom_rect(data = subset(enviro_cod, mi < quantile(mi, 0.10)),
            aes(xmin = as.numeric(as.factor(temp_bin)) - 0.5, 
                xmax = as.numeric(as.factor(temp_bin)) + 0.5,
                ymin = as.numeric(as.factor(DO_bin)) - 0.5, 
                ymax = as.numeric(as.factor(DO_bin)) + 0.5), 
...
ggpubr::ggarrange(mi_bass_1, mi_cod_1)
...
#bass
agi_bass_1 <- ggplot(enviro_bass, aes(x = as.factor(temp_bin), y = as.factor(DO_bin), fill = agi)) +
  geom_tile() + 
...
#cod
agi_cod_1 <- ggplot(enviro_cod, aes(x = as.factor(temp_bin), y = as.factor(DO_bin), fill = agi)) +
  geom_tile() + 
...
#bass large
agi_bass_lg <- ggplot(enviro_bass_size, aes(x = as.factor(temp_bin), y = as.factor(DO_bin), fill = bass_lg_agi)) +
  geom_tile() + 
...
#bass small
agi_bass_sm <- ggplot(enviro_bass_size, aes(x = as.factor(temp_bin), y = as.factor(DO_bin), fill = bass_sm_agi)) +
  geom_tile() + 
...
**1)** Identify how to incorporate changes in size into calculating the MI. Then, complete tile plots for different sized fish. 
