...
**Modified Items Since August:**

**1)** Rather than using `runif` to simulate environmental data, switch to a matrix so we have all combos of temperature and DO available.

**2)** Further explore why we saw so much variation within the AGI values across temperatures and DO. Would expect it to be a graded response, but we saw a somewhat random assortment. Seems overall that the AGI is not as good at depicting interactions among DO and temperature. With DO steps, see changes at the 4th decimal place that are likely due to temperature. 

**3)** Revise how MIcrit value was calculated. Use actual MIcrit values rather than AGI 10th percentile approach. 

**4)** Keep DO at normoxia (20.95) and use a scatterplot depicting how AGI (y-axis) varies with temperature (x-axis). 
...
Switched to MI < 1 serving as the "critical MI threshold" as seen in Slesinger et al., 2019. Please let me know if calculating MI crit should be done differently! 
...
#bass
mi_bass_1 <- ggplot(enviro_bass, aes(x = temp_whole, y = DO_whole, fill = mi)) +
  geom_tile(color = "grey60") + 
  geom_rect(data = filter(enviro_bass, mi < 1),
...
#cod
mi_cod_1 <- ggplot(enviro_cod, aes(x = temp_whole, y = DO_whole, fill = mi)) +
  geom_tile(color = "grey60") + 
  geom_rect(data = filter(enviro_cod, mi < 1),
            aes(xmin = as.numeric(temp_whole) - 1, 
                xmax = as.numeric(temp_whole) + 1,
                ymin = as.numeric(DO_whole) - 2.5, 
                ymax = as.numeric(DO_whole) + 2.5), 
